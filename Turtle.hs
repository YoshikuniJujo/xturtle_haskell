...
kameStat :: IORef (Bool, Position, Double)
kameStat = unsafePerformIO $ newIORef (True, 150, 1)
...
forward dx = do
	...
	(b, x, s) <- readIORef kameStat
	x' <- kameForward xstat b x dx $ s * 0.3
	writeIORef kameStat (b, x', s)
...
shapeSize ns = do
	...
	displayTurtle xstat x 150 (if b then 0 else 180) $ s * 0.3
	setForegroundColor xstat 0x000000
	displayTurtle xstat x 150 (if b then 0 else 180) $ ns * 0.3
	flush dpy
	writeIORef kameStat (b, x, ns)

kameForward ::
	(Display, Window, GC) -> Bool -> Position -> Position -> Double -> IO Position
kameForward xstat@(dpy, _, _) b x0 dx s = do
	doWhile x0 $ \x -> do
		setForegroundColor xstat 0xffffff
		displayTurtle xstat (x - if b then 10 else - 10) 150
			(if b then 0 else 180) s
		setForegroundColor xstat 0x000000
		displayTurtle xstat x 150 (if b then 0 else 180) s
		flush dpy
		threadDelay 30000
...
kameTurn :: (Display, Window, GC) -> Bool -> Position -> Double -> IO Bool
kameTurn xstat@(dpy, _, _) b x s = do
