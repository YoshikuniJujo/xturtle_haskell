...
import Graphics.X11.Turtle.State(TurtleState(..), getDirection, getDirectionStep)
...
moveTurtle f c l t0 t1 = do
	...
	when (visible t1) $ do
		...
		forM_ (positions t0 t1) $ \p -> flushField f $
			drawT dir1 p >> threadDelay (interval t0)
...
directions t0 t1 = case getDirectionStep t0 of
	Nothing -> []
